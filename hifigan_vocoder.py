...
def process_audio_folder(input_folder, output_folder):
    ...
    # 加载预训练的 HiFi-GAN 模型（Kokoro TTS 基于 StyleTTS 2，使用 HiFi-GAN 风格的解码器）
    hifi_gan = HIFIGAN.from_hparams(source="speechbrain/tts-hifigan-ljspeech", savedir="pretrained_models")
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(audio_extensions):
            ...
            # 确保音频是单声道并重采样到模型期望的采样率（22.05kHz）
            ...
            reconstructed_waveform = hifi_gan.decode_batch(mel).squeeze(1)
...
if __name__ == "__main__":
    ...
    output_folder = "hifigan-output"  # 输出文件夹路径
